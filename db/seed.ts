...
import { DataAPIClient, UUID, VectorDoc } from "@datastax/astra-db-ts";
...
async function main() {
 ...
 const db = client.db(process.env.ASTRA_API_ENDPOINT, { namespace: "default_keyspace"});
  ...
  products.forEach(async (product) => {
    await db.collection('products').insertOne({
        ...
        $vectorize: `${product.name}: ${product.description}`,
